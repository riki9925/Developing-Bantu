	...
	<div class="se-pre-con" id="ajxLoader" hidden=""></div>
		...
		<div class="header" id="header_berita_foto" style="background-image: url('<?php echo base_url()?>assets/img/desk.png'); background-size: cover;height: 60vh; min-height: auto">
            <div class="container" style="padding-top: 20vh; width: 80%">
                <div class="row">
					<div class="col-md-7" style="color: black">
						<p id="judul_berita_foto" style="font-weight: bold;color: #4fc143">Berita foto</p>
						<p id="deskripsi_berita_foto"></p>
					</div>
                </div>
            </div>
        </div>
		...
		<div class="main " id="main_halaman" style="margin-bottom: 30px; margin-left: 150px; margin-right: 140px; margin-top: 10px; padding-left: 30px; padding-right: 30px; background-color: transparent;">
		    	...
		    	<div class="section text-center section-landing" style="padding: 0px 0">
	    			<div class="row" style="margin-top: 30px; padding-bottom: 50px">
	    				...
	    				<div class="col-sm-10 col-xs-12" align="left" style="margin-top: 20px">
	    					<h4 style="font-weight: bold"><span id="total_foto"></span>, Foto kegiatan salingbantu.or.id</h4>
	    				</div>
	    				<div class="col-md-1 col-xs-6" style="margin-top: 15px; padding-left: 25px; ">
							<h4 id="foto_terbaru" onclick="foto_terbaru()" align="center" style="cursor: pointer;border-bottom: solid;font-weight: bold">Terbaru</h4>
						</div>

						<div class="col-md-1 col-xs-6" style="margin-top: 15px; padding-right: 25px">
							<h4 id="foto_terlama" onclick="foto_terlama()" align="center" style="cursor: pointer;">Terlama</h4>
						</div>
		...
		<!-- footer -->
		<div class="main " style="padding-left: 30px; padding-right: 30px; background-color: white;">
		    	<div class="section text-center section-landing" style="padding: 0px 0">
...
<script type="text/javascript">
	...
	$(document).ready(function() {
        ...
        if (window_width <= 768) {
           $("#header_berita_foto").css('height','unset');
           $("#judul_berita_foto").css('font-size','1.5em');
           $("#judul_berita_foto").css('font-weight','bold');
            $("#deskripsi_berita_foto").css('font-size','1.1em');
			$("#main_halaman").css('margin-left','10px');
			$("#main_halaman").css('margin-right','10px'); 
			$("#head1").hide();
			$("#navbar_desktop").hide();
			$("#navbar_mobile").show();       
		}
        else{
            $("#header_berita_foto").css('height','60vh');
            $("#judul_berita_foto").css('font-size','2.6em');
            $("#deskripsi_berita_foto").css('font-size','1.825em');
            $("#head1").show();
			$("#navbar_desktop").show();
			$("#navbar_mobile").hide();
        }
    ...
    function get_img_total(){
        $.ajax({
            url: "<?php echo site_url('Front/get_berita_total')?>",
            type: "POST",
            dataType: "JSON",
            success: function(data) {
            	$("#total_foto").text(data.total)
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $.each(data, function(k, v) {
                    ...
                    var gambar = "<div style='background:url(<?php echo base_url()?>assets/img/cover.jpg) no-repeat; background-size:cover'><div class='card' style='width: 200px; height: 200px; margin-top: 10px; margin-bottom: 10px ;border-radius: 50%; display: block;background-color: whitesmoke; margin-left:50px' align='center'><img style='width: 40%; margin-top: 50px' src='<?php echo base_url()?>assets/img/file.png'><h4>Upload foto utama</h4></div></div>";
                    

                    rows ="<div class='card' style='border-radius: 0px; margin-top: 20px; margin-bottom: 5px; padding: 0px 15px 0px 15px'><div class='row' style='background-color: rgb(22, 176, 245);padding-top: 10px'><div class='col-xs-6'><p class='category pull-left' style='color:white'><i class='material-icons'>date_range</i>"+v.waktu+"</p></div><div class='col-xs-6' style='text-align:right'><p onclick=detil_berita('mobile','"+v.id+"') class='category pull-right' style='color:white;cursor:pointer'><i class='material-icons'>launch</i></p></div></div><div class='row'><div class='col-xs-12' style='padding:0px'><h5 style='margin: 15px; text-align:left'>"+v.judul+"</h5><img class='img' src='<?php echo base_url()?>assets/img/berita/"+v.img+"' style='width: 100%'></div></div></div>";
    ...
    function foto_terbaru(){
		foto_urut = 'ASC'
		$("#foto_terbaru").css('border-bottom', 'solid');
		$("#foto_terbaru").css('cursor', 'pointer');
		$("#foto_terbaru").css('font-weight', 'bold');
		$("#foto_terlama").removeAttr('style');
		$("#foto_terlama").css('cursor', 'pointer');
		get_img();
	}

	function foto_terlama(){
		foto_urut = 'DESC'
		$("#foto_terlama").css('border-bottom', 'solid');
		$("#foto_terlama").css('cursor', 'pointer');
		$("#foto_terlama").css('font-weight', 'bold');
		$("#foto_terbaru").removeAttr('style');
		$("#foto_terbaru").css('cursor', 'pointer');
		get_img();
	}	
