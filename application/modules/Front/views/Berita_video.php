	...
	<nav id="navbar_mobile" hidden="" class="navbar navbar-default navbar-fixed-top" role="navigation" style="background-color: white; padding-bottom: 0px">
		...
		<div class="header" id="header_video" style="background-image: url('<?php echo base_url()?>assets/img/desk.png'); background-size: cover;height: 60vh; min-height: auto">
            <div class="container" style="padding-top: 20vh; width: 80%">
                <div class="row">
					<div class="col-md-7" style="color: black; margin-left: 10px">
						<p id="judul_berita_video" style="font-weight: inherit;color: #4fc143">Berita video</p>
						<p id="deskripsi_berita_video"></p>
					</div>
                </div>
            </div>
        </div>
		...
		<div class="main" id="main_halaman_video" style="margin-bottom: 30px; margin-left: 150px; margin-right: 150px; margin-top: 10px; padding-left: 30px; padding-right: 30px; background-color: transparent;">
			
		    	<div class="section text-center section-landing" style="padding: 0px 0">
	    			<div class="row" style="margin-top: 30px; padding-bottom: 50px">
						

						<div class="col-sm-10 col-xs-12" align="left" style="margin-top: 20px">
	    					<h4 style="font-weight: bold"><span id="total_video"></span>, Video kegiatan salingbantu.or.id</h4>
	    				</div>
	    				
	    				<div class="col-md-1 col-xs-6" style="margin-top: 15px; padding-left: 25px">
							<h4 id="video_terbaru" onclick="video_terbaru()" align="center" style="cursor: pointer;border-bottom: solid;font-weight: bold">Terbaru</h4>
						</div>

						<div class="col-md-1 col-xs-6" style="margin-top: 15px; padding-right: 25px">
							<h4 id="video_terlama" onclick="video_terlama()" align="center" style="cursor: pointer;">Terlama</h4>
						</div>

						<div class="col-xs-12"></div>


	    				<div class="col-md-4 col-sm-6 col-xs-12" id="kolom_1">
                        </div>
                        <div class="col-md-4 col-sm-6 col-xs-12" id="kolom_2">
                        </div>
                        <div class="col-md-4 col-sm-6 col-xs-12" id="kolom_3">
                        </div>
		...
		<div class="main" style=" margin-top: 10px; padding-left: 30px; padding-right: 30px; background-color: white;">
...
<script type="text/javascript">
	...
	$(document).ready(function() {
        ...
        if (window_width <= 768) {
           $("#header_video").css('height','unset');
           $("#judul_berita_video").css('font-size','1.5em');
           $("#judul_berita_video").css('font-weight','bold');
            $("#deskripsi_berita_video").css('font-size','1.1em');
			$("#main_halaman_video").css('margin-left','10px');
			$("#main_halaman_video").css('margin-right','10px');  
			$("#head1").hide();
			$("#navbar_desktop").hide();
			$("#navbar_mobile").show();        
		}
        else{
            $("#header_video").css('height','60vh');
            $("#judul_berita_video").css('font-size','2.6em');
            $("#deskripsi_berita_video").css('font-size','1.825em');
            $("#head1").show();
			$("#navbar_desktop").show();
			$("#navbar_mobile").hide();
        }
    ...
    function get_video_total(){
        $.ajax({
            url: "<?php echo site_url('Front/get_video_total')?>",
            type: "POST",
            dataType: "JSON",
            success: function(data) {
            	$("#total_video").text(data.total)
    ...
    function get_video(){
        $.ajax({
            ...
            dataType: "JSON",
            success: function(data) {
                $('#kolom_1').empty(); $('#kolom_2').empty(); $('#kolom_3').empty();
                var status; var color; var looping = 1; var action;

                $.each(data, function(k, v) {
                    
                	waktu = "<p class='category pull-left' style='color:white'><i class='material-icons'>date_range</i>"+v.waktu+"</p>"; 
                    action = "<p onclick='detil_kegiatan("+v.id+")' class='category pull-right' style='color:white;cursor:pointer'>DETIL<i class='material-icons'>arrow_forward</i></p>";
                    color = 'rgb(22, 176, 245)';

                    rows ="<div class='card' style='border-radius: 0px; margin-top: 20px; margin-bottom: 5px; padding: 0px 15px 0px 15px; box-shadow:0 4px 5px 0px rgba(140, 127, 127, 0.32), 0 7px 10px -5px rgb(101, 97, 97)'><div class='row' style='background-color: rgb(22, 176, 245);padding-top: 10px'><div class='col-xs-6'><p class='category pull-left' style='color:white'><i class='material-icons'>date_range</i>"+v.waktu+"</p></div><div class='col-xs-6' style='text-align:right'></div></div><div class='row'><div class='col-xs-12' style='padding:0px'><h5 style='margin: 15px; text-align:left'>"+v.deskripsi+"</h5><div style='background-color:black; padding:10px'>"+v.embed+"</div></div></div></div>";
                    
                    if (looping == 1) {
                        $(rows).appendTo("#kolom_1");
                    }
                    else if (looping == 2) {
                        $(rows).appendTo("#kolom_2");
                    }
                    else if (looping == 3) {
                        $(rows).appendTo("#kolom_3");
                        looping = 0;
                    }
    ...
    function video_terbaru(){
		video_urut = 'ASC'
		$("#video_terbaru").css('border-bottom', 'solid');
		$("#video_terbaru").css('cursor', 'pointer');
		$("#video_terbaru").css('font-weight', 'bold');
		$("#video_terlama").removeAttr('style');
		$("#video_terlama").css('cursor', 'pointer');

		get_video();
	}

	function video_terlama(){
		video_urut = 'DESC'
		$("#video_terlama").css('border-bottom', 'solid');
		$("#video_terlama").css('cursor', 'pointer');
		$("#video_terlama").css('font-weight', 'bold');
		$("#video_terbaru").removeAttr('style');
		$("#video_terbaru").css('cursor', 'pointer');
		get_video();
	}
