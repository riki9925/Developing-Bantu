...
<script type="text/javascript">
    ...
    function set_responsive_user(){
        ...
        if (layar == 'big') {
            ...
            rows = "<th style='width:70%'><?php echo $this->lang->line('daftar_user')?></th><th>Status</th><th class='disabled-sorting text-right'>Action</th>";
            $(rows).appendTo("#thead_user");
        }
    }
    ...
    function load_table(){
        table_user = $('#table_user').DataTable({ 
            ...
            // Load data for the table's content from an Ajax source
                "ajax": {
                    "url": "<?php echo site_url('Pengurus/load_table_user')?>",
                    "type": "POST",  
                    "data": function ( data ) {
                        data.layar = layar;
                        data.status = 'Semua';
    ...
    function detil_data_user(id){
        ...
        $.ajax({
                url : "<?php echo site_url('Pengurus/load_detil_user')?>",
                type: "POST",
                data: { 
                  "id" : id
                },
