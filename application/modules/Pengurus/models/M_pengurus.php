...
class M_pengurus extends CI_Model
{
    ...
    var $order_kegiatan = array('judul' => 'asc'); // default order
    ...
    public function _get_datatables_query($id3)
    {
        if ($id3 == 'kategori') {
            ...
            $this->db->select('id_kegiatan, judul, nama_kategori, target_dana, tercapai_dana ,target_hari, sisa_hari, nama, a.status');
            ...
            $this->db->select('zakatku.id, nilai_zakat, DATE_FORMAT(waktu, "%d-%m-%Y") as waktu, user.nama, user.img');
            ...
            $this->db->select('infaqku.id, nilai_infaq, DATE_FORMAT(waktu, "%d-%m-%Y") as waktu, user.nama, user.img');
